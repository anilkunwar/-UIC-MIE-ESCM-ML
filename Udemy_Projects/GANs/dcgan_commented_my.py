...
class G(nn.Module): # We introduce a class to define the generator.
    ...
    def __init__(self): # We introduce the __init__() function that will define the architecture of the generator.
        ...
        self.main = nn.Sequential( # We create a meta module of a neural network that will contain a sequence of modules (convolutions, full connections, etc.).
            ...
            nn.ConvTranspose2d(features_maps, features_maps//2, 4, 2, 1, bias = False), # We add another inversed convolution.

            # batch norm the 256 features maps.
            nn.BatchNorm2d(features_maps//2), # We normalize again.
            ...
            # 2: stride
            # 1: padding
            nn.ConvTranspose2d(features_maps//2, features_maps//4, 4, 2, 1, bias = False), # We add another inversed convolution.

            # batch norm the 128 features maps.
            nn.BatchNorm2d(features_maps//4), # We normalize again.

            nn.ReLU(True), # We apply another ReLU.

            # another transposed convolution + batch-norm + relu
            nn.ConvTranspose2d(features_maps//4,features_maps//8, 4, 2, 1, bias = False), # We add another inversed convolution.

            nn.BatchNorm2d(features_maps//8), # We normalize again.
...
class D(nn.Module): # We introduce a class to define the discriminator.
    ...
    def __init__(self): # We introduce the __init__() function that will define the architecture of the discriminator.
        ...
        self.main = nn.Sequential( # We create a meta module of a neural network that will contain a sequence of modules (convolutions, full connections, etc.).
            ...
            # 2: stride
            # 1: padding
            nn.Conv2d(3, features_maps//8, 4, 2, 1, bias = False), # We start with a convolution.
            ...
            nn.Conv2d(features_maps//8, features_maps//4, 4, 2, 1, bias = False), # We add another convolution.

            nn.BatchNorm2d(features_maps//4), # We normalize all the features along the dimension of the batch.

            nn.LeakyReLU(0.2, inplace = True), # We apply another LeakyReLU.

            nn.Conv2d(features_maps//4, features_maps//2, 4, 2, 1, bias = False), # We add another convolution.

            nn.BatchNorm2d(features_maps//2), # We normalize again.

            nn.LeakyReLU(0.2, inplace = True), # We apply another LeakyReLU.

            nn.Conv2d(features_maps//2, features_maps, 4, 2, 1, bias = False), # We add another convolution.

            nn.BatchNorm2d(features_maps), # We normalize again.
            ...
            nn.Conv2d(features_maps, 1, 4, 1, 0, bias = False), # We add another convolution.
...
for epoch in range(25): # We iterate over 25 epochs.
    ...
    for i, data in enumerate(dataloader, 0): # We iterate over the images of the dataset.
        ...
        if i % 100 == 0: # Every 100 steps:
            ...
            vutils.save_image(real, '%s/real_samples_step_%0d_epoch_%03d.png' % ("./results",i,epoch), normalize = True) # We save the real images of the minibatch.
            # get the FAKE image
            ...
            vutils.save_image(fake.data, '%s/fake_samples_step_%0d_epoch_%03d.png' % ("./results",i, epoch), normalize = True) # We also save the fake generated images of the minibatch.
